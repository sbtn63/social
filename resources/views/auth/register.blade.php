...
<div class="w-full min-h-screen bg-gray-50 flex flex-col sm:justify-center items-center pt-6 sm:pt-0">
  <div class="w-full sm:max-w-md p-5 mx-auto">
    ...
    <form action="{{ route('register') }}" method="post">
      @csrf
      <div class="mb-4">
        <label class="block mb-1" for="text">Username</label>
        @error('username') <p class="text-red-500">{{ $message }}</p> @enderror
        <input id="name" type="text" name="username" class="py-2 px-3 border border-gray-300 focus:border-red-300 focus:outline-none focus:ring focus:ring-red-200 focus:ring-opacity-50 rounded-md shadow-sm disabled:bg-gray-100 mt-1 block w-full" />
      </div>
      <div class="mb-4">
      ...
      <div class="mt-6 text-center">
        <a href="{{ route('login') }}" class="underline">Sign in for an account</a>
      </div>
    </form>
  </div>
</div>
